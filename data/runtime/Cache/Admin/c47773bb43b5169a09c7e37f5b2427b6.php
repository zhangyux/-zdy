		...
		<table class="table table-hover table-bordered">
			...
			<tbody>
				...
				<?php if(is_array($surveylist)): foreach($surveylist as $key=>$vo): ?><tr>
					<td><?php echo ($vo["questionallcount"]); ?></td>
					<td>
						<?php echo ($vo["radiotitle"]); ?>
						<!-- <?php echo ($vo["question"]); ?> -->

					</td>
					<?php if(is_array($vo['question'])): foreach($vo['question'] as $key=>$qs): ?><td><?php echo ($qs["questiontitle"]); ?>:<?php echo ($qs["count"]); ?></td><?php endforeach; endif; ?>

					
					<?php if($vo["count"] < 4): $__FOR_START_1699540927__=$vo["count"];$__FOR_END_1699540927__=4;for($i=$__FOR_START_1699540927__;$i < $__FOR_END_1699540927__;$i+=1){ ?><td></td><?php } endif; ?>
	...
	<script type="text/javascript">
		...
		function lists()
		{
			...
			var districtname = $("#district").find("option:selected").text();
			 ...
			 var suv = $("#choosesurvey").val(); 
			window.location.href="<?php echo U('Surveylist/lists');?>&survey="+suv+"&district="+district;
