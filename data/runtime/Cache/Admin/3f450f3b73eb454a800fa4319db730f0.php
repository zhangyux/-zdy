...
<body>
  ...
  <?php if(empty($question) == true): ?><div class="kzjxx_iteam">
...
<?php if(is_array($question)): foreach($question as $key=>$v): ?><div class="kzjxx_iteam">
                            <input name="" type="radio" value="" class="dxk">
	...
	<script type="text/javascript">
		...
		function sub()
		{	var radioid = "<?php echo ($radio["radioid"]); ?>";
			var getquestion = $(".dx_box").children("textarea").val(); //编辑题目区
			  $(".dx_box").children(".title_itram").children('.kzjxx_iteam').each(function(){
				...
				$.getJSON("<?php echo U('Surveylist/edit_post');?>",{radioid:radioid,question:question},function(data){
					// alert(data)
				});
			});  
				$.getJSON("<?php echo U('Surveylist/edit_posttitle');?>",{radioid:radioid,question:getquestion},function(data){
					// alert(data)
